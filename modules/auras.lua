...
function Auras:GetOptions()
				...
				newAura = {
					...
					args = {
						...
						add = {
							...
							name = L["Add new Aura"],
							func = function(info)
								self.db.aurasFrameAuras[Auras.newAuraName] = Auras.newAuraPriority
	...
	-- set auras
	for aura, priority in pairs(self.db.aurasFrameAuras) do
...
local function setAura(info, value)
	if (info[#(info)] == "name") then
		-- create new aura
		Auras.db.aurasFrameAuras[value] = Auras.db.aurasFrameAuras[info[#(info) - 1]]

		-- delete old aura
		Auras.db.aurasFrameAuras[info[#(info) - 1]] = nil
	else
		Auras.db.aurasFrameAuras[info[#(info) - 1]] = value
	end
end
...
local function getAura(info)
	if (info[#(info)] == "name") then
		...
		return Auras.db.aurasFrameAuras[info[#(info) - 1]]
	end
end
	...
	return {
		...
		args = {
			...
			delete = {
				...
				name = L["Delete"],
				func = function(info)
					self.db.aurasFrameAuras[info[#(info) - 1]] = nil
