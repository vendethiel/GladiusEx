...
local Trinket = Gladius:NewGladiusModule("Trinket", false, {
...
function Trinket:GetModuleAttachPoints()
   return {
      ["Trinket"] = L["Trinket"],
   }
end

function Trinket:GetAttachFrame(unit)
...
function Trinket:UpdateTrinket(unit, duration)
   ...
   if (Gladius.db.announcements.trinket) then
      -- todo: replace with a sendmessage
      Gladius:GetModule("Announcements"):Send(strformat(L["TRINKET USED: %s (%s)"], UnitName(unit) or "test", UnitClass(unit) or "test"), 2, unit)   
   end
   ...
   if (Gladius.db.announcements.trinket or Gladius.db.trinketGridStyleIcon) then
      ...
      self.frame[unit]:SetScript("OnUpdate", function(f, elapsed)
         ...
         if (self.frame[unit].timeleft <= 0) then
            ...
            -- announcement
            if (Gladius.db.announcements.trinket) then
               Gladius:GetModule("Announcements"):Send(strformat(L["TRINKET READY: %s (%s)"], UnitName(unit) or "", UnitClass(unit) or ""), 2, unit)
            end
...
function Trinket:Update(unit)   
   ...
   -- anchor point 
   local parent = Gladius:GetAttachFrame(unit, Gladius.db.trinketAttachTo)     
            ...
            position = {
               ...
               args = {
                  trinketAttachTo = {
                     ...
                     values=function() return Trinket:GetAttachPoints() end,
                     disabled=function() return not Gladius.dbi.profile.modules[self.name] end,
                     arg="general",
                     order=5,
                  },
